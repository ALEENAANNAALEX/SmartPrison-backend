...
router.get('/stats', requireAdmin, async (req, res) => {
  try {
    ...
    const totalStaffOnly = await User.countDocuments({ role: 'staff' });
    const totalBlocks = await PrisonBlock.countDocuments({ $or: [{ isActive: true }, { isActive: { $exists: false } }] });
    ...
    res.json({
      ...
      totalWardens: totalWardens || 0,
      totalStaff: totalStaffOnly || 0,
...
router.get('/dashboard/stats', requireAdmin, async (req, res) => {
  try {
    const stats = {
      ...
      // Count blocks that are active or missing the flag (legacy docs)
      totalBlocks: await PrisonBlock.countDocuments({ $or: [{ isActive: true }, { isActive: { $exists: false } }] }),
      // Count all wardens; legacy docs may not have wardenDetails.isActive
      totalWardens: await User.countDocuments({ role: 'warden' }),
...
router.post('/wardens', requireAdmin, async (req, res) => {
  try {
    ...
    if (experience && experience !== '') {
      ...
      if (isNaN(exp) || exp < 0 || exp > 39) {
        return res.status(400).json({ msg: 'Experience must be a valid number between 0 and 39' });
      }
    }
    ...
    // Send email with auto-generated password (reuse robust service)
    try {
      const { sendWardenWelcomeEmail } = require('../services/wardenEmailService');
      await sendWardenWelcomeEmail(
        { name, email, phone },
      ...
      console.error('❌ Failed to send warden welcome email:', emailError);
...
router.post('/prisoners', requireAdmin, uploadPrisonerFiles, handleUploadError, async (req, res) => {
  try {
    ...
    // Handle emergency contact government ID files with indexed field names
    const emergencyContactGovIdFiles = {};
    if (req.files) {
      Object.keys(req.files).forEach(key => {
        if (key.startsWith('emergencyContactGovernmentId_')) {
          const index = key.split('_')[1];
          const file = req.files[key][0];
          if (file) {
            emergencyContactGovIdFiles[index] = {
              path: `/uploads/prisoner-docs/${path.basename(file.path)}`,
    ...
    console.log('📁 Emergency contact government ID files received:', Object.keys(emergencyContactGovIdFiles).length);
    if (Object.keys(emergencyContactGovIdFiles).length > 0) {
      console.log('📁 Files:', emergencyContactGovIdFiles);
    }
    ...
    // Debug: Log emergency contact data being processed
    console.log('🔍 DEBUG: Normalized emergency contacts for create:', normalizedEmergencyContacts);
    console.log('🔍 DEBUG: Emergency contact government ID files for create:', emergencyContactGovIdFiles);
    
    // Assign government ID files to emergency contacts
    Object.keys(emergencyContactGovIdFiles).forEach(index => {
      const fileInfo = emergencyContactGovIdFiles[index];
      const contactIndex = parseInt(index);
      if (normalizedEmergencyContacts[contactIndex]) {
        normalizedEmergencyContacts[contactIndex].governmentId = fileInfo.path;
        console.log(`📁 Assigned file ${fileInfo.path} to emergency contact ${contactIndex}: ${normalizedEmergencyContacts[contactIndex].name}`);
      }
    });
    
    console.log('🔍 DEBUG: Final emergency contacts with government IDs:', normalizedEmergencyContacts);
    ...
    // Handle government validation data if provided
    let governmentValidationData = {};
    if (req.body.governmentValidation) {
      try {
        governmentValidationData = JSON.parse(req.body.governmentValidation);
      } catch (e) {
        console.warn('Invalid government validation data:', e.message);
      }
    }
    ...
    const newPrisoner = new Prisoner({
      ...
      governmentValidation: {
        isVerified: governmentValidationData.isVerified || false,
        verifiedAt: governmentValidationData.verifiedAt ? new Date(governmentValidationData.verifiedAt) : undefined,
        verifiedBy: governmentValidationData.verifiedBy || req.user._id,
        discrepancies: governmentValidationData.discrepancies || [],
        validationStatus: governmentValidationData.validationStatus || 'pending',
        overrideReason: governmentValidationData.overrideReason,
        overrideApprovedBy: governmentValidationData.overrideApprovedBy || req.user._id,
        overrideApprovedAt: governmentValidationData.overrideApprovedAt ? new Date(governmentValidationData.overrideApprovedAt) : undefined
      }
    });
...
router.put('/prisoners/:id', requireAdmin, uploadPrisonerFiles, handleUploadError, async (req, res) => {
  try {
    ...
    // Handle emergency contact government ID files for updates with indexed field names
    const emergencyContactGovIdFiles = {};
    console.log('🔍 DEBUG: req.files received:', req.files ? Object.keys(req.files) : 'No files');
    if (req.files) {
      Object.keys(req.files).forEach(key => {
        console.log(`🔍 DEBUG: Processing file field: ${key}`);
        if (key.startsWith('emergencyContactGovernmentId_')) {
          const index = key.split('_')[1];
          const file = req.files[key][0];
          console.log(`🔍 DEBUG: Emergency contact file ${index}:`, file ? file.originalname : 'No file');
          if (file) {
            emergencyContactGovIdFiles[index] = {
              path: `/uploads/prisoner-docs/${path.basename(file.path)}`,
              originalName: file.originalname
            };
            console.log(`📁 DEBUG: Stored emergency contact file ${index} with path:`, emergencyContactGovIdFiles[index].path);
          }
        }
      });
    }
    
    console.log('📁 UPDATE: Emergency contact government ID files received:', Object.keys(emergencyContactGovIdFiles).length);
    if (Object.keys(emergencyContactGovIdFiles).length > 0) {
      console.log('📁 UPDATE: Files:', emergencyContactGovIdFiles);
    }
    
    // Get existing prisoner data to preserve existing government ID paths
    const existingPrisoner = await Prisoner.findById(req.params.id);
    const existingEmergencyContacts = existingPrisoner?.emergencyContacts || [];
    
    // Debug: Log emergency contacts data being processed
    console.log('🔍 DEBUG: Emergency contacts data in update:', update.emergencyContacts);
    console.log('🔍 DEBUG: Existing emergency contacts:', existingEmergencyContacts);
    
    if (photoPath) update.photograph = photoPath;
    if (governmentIdPath) update.governmentId = governmentIdPath;
    
    // Assign emergency contact government ID files
    console.log('🔍 DEBUG: Assigning emergency contact government ID files...');
    Object.keys(emergencyContactGovIdFiles).forEach(index => {
      const fileInfo = emergencyContactGovIdFiles[index];
      const contactIndex = parseInt(index);
      console.log(`🔍 DEBUG: Processing file for contact index ${contactIndex}:`, fileInfo);
      if (update.emergencyContacts && update.emergencyContacts[contactIndex]) {
        console.log(`🔍 DEBUG: Before assignment - contact ${contactIndex} governmentId:`, update.emergencyContacts[contactIndex].governmentId);
        update.emergencyContacts[contactIndex].governmentId = fileInfo.path;
        console.log(`🔍 DEBUG: After assignment - contact ${contactIndex} governmentId:`, update.emergencyContacts[contactIndex].governmentId);
        console.log(`📁 UPDATE: Assigned file ${fileInfo.path} to emergency contact ${contactIndex}: ${update.emergencyContacts[contactIndex].name}`);
      } else {
        console.log(`🔍 DEBUG: No emergency contact found at index ${contactIndex}`);
      }
    });
    
    // Preserve existing government ID paths for contacts that don't have new files uploaded
    // Only preserve if the governmentId field is not explicitly set to empty string
    if (Array.isArray(update.emergencyContacts)) {
      update.emergencyContacts.forEach((contact, index) => {
        if (contact.governmentId === '' || contact.governmentId === undefined || contact.governmentId === null) {
          if (existingEmergencyContacts && existingEmergencyContacts[index] && existingEmergencyContacts[index].governmentId && contact.governmentId !== '') {
            contact.governmentId = existingEmergencyContacts[index].governmentId;
            console.log(`📁 UPDATE: Preserved existing government ID for emergency contact ${index}: ${contact.name}`);
          } else if (contact.governmentId === '') {
            console.log(`📁 UPDATE: Explicitly removed government ID for emergency contact ${index}: ${contact.name}`);
          }
        }
      });
    }
    
    // Debug: Log final emergency contacts data
    console.log('🔍 DEBUG: Final emergency contacts data before save:', update.emergencyContacts);
    ...
    console.log('🔍 DEBUG: About to save prisoner with update data:', JSON.stringify(update, null, 2));
    console.log('🔍 DEBUG: Emergency contacts in update data:', update.emergencyContacts);
    ...
    console.log('🔍 DEBUG: Prisoner after update:', prisoner ? 'Successfully updated' : 'Update failed');
    if (prisoner && prisoner.emergencyContacts) {
      console.log('🔍 DEBUG: Emergency contacts after save:', prisoner.emergencyContacts);
      prisoner.emergencyContacts.forEach((contact, index) => {
        console.log(`🔍 DEBUG: Emergency contact ${index}:`, {
          name: contact.name,
          governmentId: contact.governmentId
        });
      });
    } else {
      console.log('🔍 DEBUG: No emergency contacts found after save');
    }
    ...
    const ecFilesCount = Object.keys(emergencyContactGovIdFiles).length;
    let msg = 'Prisoner updated successfully';
    if (ecFilesCount > 0) {
      msg += ` with ${ecFilesCount} emergency contact government ID file(s)`;
    }
    
    res.json({ success: true, prisoner, msg });
  } catch (error) {
